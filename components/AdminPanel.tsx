...
import { User, UserRole, DieCut, FieldKey, JumboPreset } from '../types';
import { TrashIcon, EyeIcon, EyeOffIcon, SaveIcon } from './IconComponents';
...
const AdminPanel: React.FC = () => {
    const { 
        ...
        jumboPresets, addJumboPreset, removeJumboPreset,
    ...
    const [activeTab, setActiveTab] = useState<'users' | 'notifications' | 'lists' | 'dies' | 'inventory' | 'jumbos' | 'config'>('users');
    ...
    // JUMBO PRESET FORM
    const [newJumboPreset, setNewJumboPreset] = useState<Partial<JumboPreset>>({});
    ...
    // --- JUMBO PRESETS HANDLER ---
    const handleAddJumboPreset = (e: React.FormEvent) => {
        e.preventDefault();
        if (newJumboPreset.materialCode && newJumboPreset.defaultWidth && newJumboPreset.defaultLength) {
            addJumboPreset({
                id: `preset-${Date.now()}`,
                materialCode: newJumboPreset.materialCode,
                defaultWidth: Number(newJumboPreset.defaultWidth),
                defaultLength: Number(newJumboPreset.defaultLength)
            });
            setNewJumboPreset({});
        }
    };
    ...
    return (
        <div className="space-y-6 pb-10">
            <div className="flex flex-wrap gap-2 sm:gap-4 border-b border-gray-200 dark:border-gray-700 pb-2 overflow-x-auto">
                ...
                <button onClick={() => setActiveTab('jumbos')} className={`px-4 py-2 rounded-t-lg font-medium ${activeTab === 'jumbos' ? 'bg-orange-600 text-white' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700'}`}>Config. Jumbos</button>
            ...
            {activeTab === 'jumbos' && (
                <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
                    <h2 className="text-2xl font-bold dark:text-white mb-4">Configuración Predeterminada de Jumbos</h2>
                    <p className="text-sm text-gray-500 mb-6">Define el ancho y largo estándar de los jumbos por familia de material. Estos valores se usarán automáticamente al generar órdenes de compra.</p>
                    
                    <form onSubmit={handleAddJumboPreset} className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6 bg-gray-50 dark:bg-gray-900 p-4 rounded border border-gray-200 dark:border-gray-700">
                        <div>
                            <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Cód. Familia (3 dig)</label>
                            <input 
                                type="text" 
                                value={newJumboPreset.materialCode || ''} 
                                onChange={e => setNewJumboPreset({...newJumboPreset, materialCode: e.target.value})} 
                                placeholder="Ej: 260" 
                                className="w-full p-2 border rounded dark:bg-gray-700 dark:text-white font-mono"
                                maxLength={3}
                            />
                        </div>
                        <div>
                            ...
                            <input 
                                type="number" 
                                value={newJumboPreset.defaultWidth || ''} 
                                onChange={e => setNewJumboPreset({...newJumboPreset, defaultWidth: parseFloat(e.target.value)})} 
                                placeholder="Ej: 1500" 
                                className="w-full p-2 border rounded dark:bg-gray-700 dark:text-white"
                            />
                        </div>
                        <div>
                            <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Largo Jumbo (m)</label>
                            <input 
                                type="number" 
                                value={newJumboPreset.defaultLength || ''} 
                                onChange={e => setNewJumboPreset({...newJumboPreset, defaultLength: parseFloat(e.target.value)})} 
                                placeholder="Ej: 3000" 
                                className="w-full p-2 border rounded dark:bg-gray-700 dark:text-white"
                            />
                        </div>
                        <div className="flex items-end">
                            <button type="submit" className="w-full py-2 bg-green-600 hover:bg-green-700 text-white rounded font-bold flex items-center justify-center gap-2">
                    ...
                    <table className="w-full text-left text-sm dark:text-white">
                        <thead className="bg-gray-100 dark:bg-gray-700">
                            <tr>
                                ...
                                <th className="p-3 text-right">Ancho Predet.</th>
                                <th className="p-3 text-right">Largo Predet.</th>
                                <th className="p-3 text-right">Acciones</th>
                            </tr>
                        </thead>
                        <tbody>
                            {jumboPresets.length === 0 ? (
                                <tr><td colSpan={4} className="p-4 text-center text-gray-500">No hay configuraciones guardadas.</td></tr>
                            ) : jumboPresets.map(preset => (
                                <tr key={preset.id} className="border-b dark:border-gray-700">
                                    <td className="p-3 font-mono font-bold bg-orange-50 dark:bg-orange-900/20 text-orange-800 dark:text-orange-200">{preset.materialCode}</td>
                                    <td className="p-3 text-right">{preset.defaultWidth} mm</td>
                                    <td className="p-3 text-right">{preset.defaultLength.toLocaleString()} m</td>
                                    <td className="p-3 text-right">
                                        <button onClick={() => removeJumboPreset(preset.materialCode)} className="text-red-500 hover:text-red-700">
