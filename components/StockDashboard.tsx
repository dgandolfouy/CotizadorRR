...
const StockDashboard: React.FC = () => {
    ...
    const generateOrderPDF = async () => {
        ...
        try {
            ...
            doc.setFontSize(14);
            doc.text("MATERIA PRIMA (JUMBOS)", 200, 27, { align: "right" });
            ...
            pendingJumbos.forEach((jumbo, idx) => {
                ...
                doc.text(`Ancho Jumbo: ${jumbo.jumboWidth}mm  |  Bajadas (Jumbos): ${runs}`, 20, y + 11);
        ...
        <div className="p-4 sm:p-8 bg-gray-50 dark:bg-gray-900 min-h-screen">
            ...
            {showReportModal && (
                <div className="fixed inset-0 bg-black bg-opacity-80 z-50 flex justify-center items-center p-2 sm:p-4">
                    <div className="bg-white dark:bg-gray-900 rounded-xl shadow-2xl w-full max-w-6xl border border-gray-200 dark:border-gray-700 flex flex-col h-[90vh]">
                        ...
                        <div className="p-4 sm:p-6 flex-1 overflow-y-auto bg-gray-50 dark:bg-gray-900/50">
                            {reportType === 'inventory' && ( <div className="flex flex-col items-center justify-center h-full space-y-6"><div className="text-center max-w-lg"><CubeIcon className="h-16 w-16 mx-auto text-gray-300 mb-4" /><h3 className="text-xl font-bold dark:text-white">Generar Reporte de Inventario</h3><p className="text-gray-500 mt-2">Descarga un PDF detallado.</p></div><button className="bg-orange-600 hover:bg-orange-700 text-white px-8 py-3 rounded-lg font-bold shadow-lg flex items-center gap-2"><DownloadIcon className="h-5 w-5" /> Descargar PDF Inventario</button></div> )}
                                    ...
                                    <div className="bg-white dark:bg-gray-800 p-4 sm:p-6 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
                                        <div className="flex flex-col lg:flex-row gap-8">
                                            ...
                                            <div className="w-full lg:w-1/3 space-y-4">
                                                ...
                                                <div className="flex flex-col gap-2">
                                                    <button onClick={addJumboToOC} className={`flex-1 py-3 rounded-lg font-bold shadow-lg text-sm flex items-center justify-center gap-2 text-white ${editingId ? 'bg-blue-600 hover:bg-blue-700' : 'bg-gray-900 hover:bg-black'}`}><SaveIcon className="h-4 w-4" /> {editingId ? 'Actualizar Ítem' : 'Agregar a la OC'}</button>
                                ...
                                <div className="space-y-4">
                                    {ocHistory.length === 0 ? <div className="text-center py-20 text-gray-400">No hay historial.</div> : ocHistory.map(order => (
                                        <div key={order.id} className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow border border-gray-200 dark:border-gray-700 flex justify-between items-center">
                                            <div><h4 className="font-bold text-blue-600 dark:text-blue-400 text-lg">{order.id}</h4><p className="text-xs text-gray-500">{new Date(order.date).toLocaleString()} - {order.items.length} Items</p></div>
                                            <div className="flex gap-2">
